...
* The show must go on: Can private art museums survive?
...
#
Using a newly constructed database of {{{pm_n_used}}} museums and {{{pm_n_closed}}} closures ({{{n_pmyears}}} museum years), I use Cox Proportional Hazard models to investigate which factors increase the chance of private museum closure.
#
...
#+latex: \bigbreak
#+latex: \noindent
*Keywords*: closing, museum, non-profit organization, leadership dependence, diversification, identity, competition
...
#
Many aspects of this new organizational form have attracted attention, such as their emergence, relations to other institutions or impact on artist consecration (cf. [[textcite:&Kolbe_etal_2022_privatemuseum]] for a literature review).
#
...
#  but also into the ongoing processes of a private museums.
# , such as dependency relationships.
#
...
A recent study by [[textcite:&Velthuis_Gera_2024_fragility]] has systematically investigated closing reasons of private museums, and I build on this research by using a more elaborate statistical model and additional data sources.
#
...
#
# Taken together, the results provide little evidence that private museums are overly dependent on their original founder, nor that they face higher mortality rates than other non-profit museums.
# #
# While little evidence points toward private museums being involuntarily at risk due to their low diversification,
...
# The role of the founder in museum closure may thus be more limited to voluntary closures, as evidenced by higher closing rates for museums with unconventional names (in particular, museums without the term "museum", "foundation" or "collection" in the name), which however may also indicate an effect of limited recognition by other art-field actors. 
# 
# Private museums with unconventional names (in particular, without the term "museum" in the name) face a higher closing risk, which may indicate either a temporary intention by their founders from the very beginning and/or limited recognition by third parties as potential closing reasons.
# 
...
*** Financial conditions in the cultural sector
...
Given that operating at "the margin of financial sustainability" is the "ordinary condition" for non-profit organizations (NPO) in the cultural sector, [[parencite:&Licci_BaraldiBonini_2024_sustainability p.2]], it is not surprising that costs have been listed as a primary reason for private museum closure:
# 
...
*** The risks of relying on single founders
...
# 
[[textcite:&Adam_2021_rise]] argues that "ultimately, many of these failures demonstrate the fragility of spaces that rely on a single founder, whose motivations and financial stability may change, or who may not have realized the difficulties inherent in establishing their own art space" (p.82).
# 
...
#
[[textcite:&Bechtler_Imhof_2018_future]] also characterize "the dependency on single individuals only" as one of the main reasons that "make these private museums fragile" (p.53).
#
...
# 
[[textcite:&Adam_2021_rise]] argues that succession introduces financial strains as "collectors’ successors may not look kindly on what may well be a money pit when [...] they will have to pick up the bills" (p.77), and also highlights the aesthetic dimension: "The nature of contemporary art is to be fresh and new, but what is ‘fresh and new’ for the founder might seem ‘old hat’ to their heirs". (p.76).
#
...
# 
textcite:&Hager_1999_demise reports that "both individual donations/subscriptions and community and corporate grants tended to go to the singular, most reputable arts organizations of their types" (p.173), resulting in "strong competitive pressures" for the vast majority of organizations (ibid.).
#
...
#
Furthermore, a small number of museums are excluded, in particular {{{pmdb_n_closed_NAclosingyear}}} museums for which the closing date could not be found, {{{pm_open_from2021}}} museums which opened in 2021 and later (as survival analysis requires positive survival times), {{{pmdb_n_open_NAopeningyear}}} museum the opening year of which could not be determined, as well as {{{pmdb_n_closed_before2k}}} museums which closed before 2000 and thereby fall outside the period under investigation.
#
...
Self-identification measures whether the name of a museum includes the term "Museum", "Foundation", "Collection", or other (such as "Villa", "Institute", "Center" or "Gallery", which are grouped together as they are less frequent, see table {{{reftbl_t_selfid}}}).
#
...
#
While this operationalization is limited to private museums in the neighborhood, rather than the more complete set of all museums private museums may compete with, it is by no means a rarity to find private museums within a 10 km radius, as in {{{pmyear_proxcnt10_geq0}}}% museum-years one or more private museums in the surroundings are observed. 
...
- textcite:&Gordon_etal_2013_insolvency observe 35 closures based on 5,167 museum-years between 2000 and 2003 (p.368), resulting in an average annual closing rate of 0.68% (p.377)[fn::The difference to [[textcite:&Bowen_etal_1994_charitable]] is likely due to exclusion of organizations with revenues below 150,000 USD.].
...
# 
The insignificance of the coefficients of the external shocks indicates that museums are overall relatively resilient; possibly unlike other types of art organizations with lower assets and higher reliance on earned incomes [[parencite:&Bowen_etal_1994_charitable p.102]].  
...
The death of the founder is not associated with a significant difference (p={{{pvlu_founder_dead_binary}}}) in private museum closure, i.e. museums are not significantly more likely to close after their founder has died, which provides little support to H1a.[fn::An alternative specification of founder death which partitions the time after the death into a short-term and long-term period is reported in the supplementary materials; this specification also provides little evidence for a significant effect of the death of the founder.]
#
...
# Other founder-level characteristics also do not indicate that founder-related variables are strongly associated to museum closure, as the coefficients of ARTnews ranking inclusion (measuring reputation) are not statistically significant.
#
...
#
Some support for the latter interpretation is given by the fact that two of the three museums for which [[textcite:&Velthuis_Gera_2024_fragility]] identify a temporal intention as the closing reason do not self-identify as museum, collection or foundation.
#
# Temporal intentions are however the exceptions among closing reasons identified by [[textcite:&Velthuis_Gera_2024_fragility]] (p.6), and as similarly only a fraction of private museum chooses such a name ({{{N_slfid_other}}} museums, or {{{perc_slfid_other}}}%), the overall proportion of these types of voluntary closures compared to all closures is presumably limited.
...
#
Yet at the same time, at low private museum numbers (0 to 3 private museums, left half of figure {{{refplt_p_pred_heatmap}}}), increases in population also increase mortality; this range also includes a large number of museum-years ({{{N_zoneleft}}} museum years; {{{perc_zoneleft}}}% of the data).
#
...
Taken together, figure {{{refplt_p_pred_heatmap}}} indicates two niches as particularly supportive of private museum survival, on the one hand, remote areas with low population and no competing private museums, and on the other densely populated areas with at least some other private museums.
...
# Previously the dozens of closures have been seen as indicator of their vulnerability and short-livedness [[parencite:&Adam_2020_close]], however this analysis shows the importance of studying closing not in isolation but in comparison to both non-closures and closures of comparable institutions.
# #
...
# Comparing private museums closures to closures of other organizational form advances the understanding of the sustainability of this new organizational form.
# #
...
# 
So far only few museums have experienced the death of their founder ({{{n_founder_dead}}} museums, {{{perc_founder_dead}}}%), one of the primary reasons hypothesized to lead to the private museum closure. 
#
