...
#
German museums in the 1990s covered on average only around 30% of their budget by their income [[parencite:&Martin_1993_museen]][fn::Own calculations based on table p.233], with private museums requiring less support as operations provide around 47% of their budget (ibid.), and even the more market-oriented NPOs in the cultural sector draw around 50% of their funding from public support [[parencite:&Zimmer_Priller_2007_gemeinnuetzig p.82]].
#
...
#
While the question of which mechanism explains the association between name and survival, the role of the environment can be clearly seen in terms of nearby institution counts and population measures, with both factors leading to higher closing rates.
...
#
Most national public flagship institutions will presumably remain sufficiently funded to survive, however this might not be the case for the much larger of smaller museums in less touristic regions who rely on more limited municipal funding ({{{own_loc_reg_perc}}}% of the European museum population, [[cite:&EGMUS_2024_complete]]), as textcite:&StylianouLambert_etal_2014_museums explore for a number of smaller publicly funded ethnography museums in Cyprus.
#
Yet without access to museum registers comparable to the IRS/NCCS datasets, questions regarding the relation between type of ownership and survival remain speculative.
#
...
Additionally, this analysis contributes to the understanding of environmental influences of museum survival, in particular competition, which has been mentioned in the private museum literature generally but has not specifically linked to survival.
#
Here, a complex picture emerges as survival aspects are in most cases limited by both higher competition (which was expected) and audiences (which was unexpected); while a the reverse is the case for smaller sections of the sample.
# idk what else to write here, besides it's complex and unexpected
The fact that such patterns might be explained by geography-dependennt variation in founder commitment, which has not been observed, shows how central the role of the founder might be.??????????????
