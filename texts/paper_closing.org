...
#
Furthermore, the focus that has been given to questions of sustainability has been primarily conducted through case studies; beyond the exception of [[textcite:&Velthuis_Gera_2024_fragility]] quantitative studies have been absent.
# 
...
#
German museums in the 1990s covered on average only 27% of their expenses covered by operations [[parencite:&Martin_1993_museen p.233]], and even the more market-oriented NPOs in the cultural sector draws around 50% of their funding from public support [[parencite:&Zimmer_Priller_2007_gemeinnuetzig p.82]].
...
#
Furthermore, a small number of museums are excluded, in particular {{{pmdb_n_closed_NAclosingyear}}} museums for which the closing date could not be found, {{{pm_open_from2021}}} museums which opened in 2021 and later (as survival analyses requires positive survival times), {{{pmdb_n_open_NAopeningyear}}} museum the opening year of which could not be determined, as well as {{{pmdb_n_closed_before2k}}} which closed before 2000 thereby fall outside the period under investigation.
#
...
# 
This non-parametric method estimates the hazard rate as the conditional probability of exiting at time t_i given being alive as $h(t_i) = \frac{d_i}{n_i}$, (with $n_i$ as the number of units at risk at time $t_i$ and $d_i$ as the number of exits at time $t_i$).
# 
...
*** Comparing private museum hazard rates to previous studies
:PROPERTIES:
...
#
Summarizing yearly closing proportions results in an average closing rate of {{{yearhaz_all}}}; i.e. on average each year {{{yearhaz_all}}} of the private museum population closes (weighing the yearly mortality rate by number of museums at risk results in an estimate of {{{yearhaz_allw}}}).
#
...
#+caption: Hazard rates in studies of museums survival
#+label: tbl:litreview
|-----------------------------------+---------------------+-------------------------|
...
#
None of these focus on art museums exclusively ([[textcite:&Hager_2001_vulnerability]] focuses on Art NPOs, [[textcite:&Bowen_etal_1994_charitable]] and [[textcite:&Gordon_etal_2013_insolvency]] on NPOs generally), but report survival estimates by organizational form, including museums.
#
[[textcite:&Bowen_etal_1994_charitable]] investigates the closing of 501(c)(3) museums in the 1981-1991 time period and, based on 271 (9.8%) closures of a population of 2755, calculate average annual exit rates as 1.1% (p.103).
#
# (rather than only institutions with revenues exceeding 25,000 USD).
#
[[textcite:&Gordon_etal_2013_insolvency]] observes 35 closures over the years of 2000-2003 with 5,167 museum-years (p.368), resulting in an average annual closing rate of 0.68% (p.377).
#
Some of this difference may be explained by higher shares of smaller organizations in [[textcite:&Bowen_etal_1994_charitable]], where museums of any size are included, whereas [[textcite:&Gordon_etal_2013_insolvency]] excluded NPOs with operational revenues below 150,000 USD, as smaller non-profits are at higher risk of closing [[parencite:&Harrison_Laincz_2008_nonprofit;&Helmig_Ingerfurth_Pinz_2013_nonprofit]].


Focusing directly on Art museums, rather than museum generally as part of the wider focus on Art NPOs, [[textcite:&Hager_2001_vulnerability]] observes 42 closures over a period of 4 years (1994-1997) from a starting populaton of 448 (p.383).
#
As his way of delineating the population (having "filed an annual Form 990 tax return with the IRS for the 1990, 1991, or 1992 tax year", p.380) does not rule out that some closings took place in 1991, 1992 or 1993[fn::In particular it is unclear why 1993 is not treated as belonging to the observation period], attributing some closings to these years is likely appropriate, and presumably roughly equivalent to assuming an observation period of five (rather than four) years, which corresponds to an average yearly closing rate of 1.9%.[fn::So far it is hard to tell why art museums appear to have higher closing rates than museums generally.
#
According to the [[textcite:&IMLS_2008_funding]], art museums have above-average reliance on earned income (46.1% compared to 43.7% on average, p.26), and [[textcite:&Bowen_etal_1994_charitable]] argue that a higher reliance on income is associated with higher exit rates (p.102), but it is questionable whether such an a small difference in percent earned can explain a 1.7-2.7 times higher closing risk.
#
...
#
Especially the larger number of history museums, and the more restrictive inclusion by [[textcite:&Hager_2001_vulnerability]] in comparison to [[textcite:&Bowen_etal_1994_charitable]] make the higher mortality rate for art museums than museums generally puzzling.] 
...
#
Yet at the same time, at low private museums numbers (0 to 3 private museums), increases in population also increase mortality; this range also includes a large number of museum-years ({{{N_zoneleft}}} museum years; {{{perc_zoneleft}}}% of the data).
#
...
The overall picture that emerges from the analysis is complex.
#
While private museums do close, they do not so at higher rates at other non-profit museums
#
A possible reason for that is that founder-level variables, in particular the founder's death, are not significantly associated with closing that is unvoluntary.
#
While so far little evidence points toward low diversification, i.e. reliance on the founder as a key vulnerability of private museums, the longevity of a subgroup of private museums might be limited by the temporary intentions of their founders.
#
Beyond founder-reliance, private museums depend substantially on their environment, which additional private museums in their surrounding constituting an increase in closing risk for the majority. 



While previously the dozens of closures have been seen as indicator of their vulnerability and short-livedness [[parencite:&Adam_2020_close]], this analysis shows the importance of establishing a proper comparisons.
#
While the booming numbers of private museums themselves have received substantial attention, less consideration has been given to the fact that large populations can generate dozens of closures without requiring high closing rates, especially when considered over a span of several years.
#
In other words, the sheer size to which the the private museum population has grown over the last decades has made it all but inevitable that some of them would close, and do so after only a few years.
#
Based on the current analysis, private museums will continue to close in the years to come, at hazard rates of around 1% a handful of closures every year are to be expected. 

# FIXME better integration to previous paragraph
While overall private museum closing rates are generally not diverging from museums in general, and may even lie below the average, it has to be kept in mind that this comparison is based on American non-profit museums.
#
To the best of my knowledge, currently no data sources exist that allow estimation of closing rates of (European) public, state-funded museums.
#
...
#
While most national public flagship institutions will likely remain sufficiently funded to survive, this might not be the case for the vast majority of smaller museums in less touristic regions who rely on limited municipal funding, as textcite:&StylianouLambert_etal_2014_museums demonstrate for a number of smaller publicly funded ethnography museums in Cyprus.
#
Yet without access to museum register comparable to the IRS/NCCS datasets, such questions regarding the relation between type of ownership and survival remain speculative.




However, it is necessary to consider that so far only a small fraction ({{{n_founder_dead}}} museums, {{{perc_founder_dead}}}%) of museums has experienced the death of their founder, one of the primary reasons hypothesized to lead to the private museum closure. 
#
...
# 
As the currently large confidence interval (figure {{{refplt_p_surv_death}}}) for museums whose founders have died allows a wide range of values, it is entirely possible that previous scholars have indeed correctly identified a key dependency (the current absence of evidence of it should not be taken as evidence of absence) and that therefore later analyses will a significantly association between founder death and closing risk. 
#
