...
#
While the coefficient in the short-term period is consistently higher than both the overall death coefficient and the long-term period coefficients, so far the evidence is inconclusive as only few cases are statistically significant[fn:: P-values are not adjusted for multiple tests].
#
Considering the underlying numbers supports this:
# 
While {{{n_founder_dead}}} private museums have seen the death of their founders, only {{{n_pm_close_after_recent_death}}} of these have closed in the following 5 years; observing statistical significance in some configurations of founder death time more likely reflect random clustering than an underlying pattern.
#

*Effect sizes*:
- death overall
- short-term death
stress how few people it is
