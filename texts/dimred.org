...
overall goal: get some size indicator, or measure 
...
Whether or not the principal components are included matters somewhat: the main variables that were significant before (self-identification as other), nbr PM within 10km, and its interaction with population, are not much changed.

MOW is not significant anymore when both PCs are included, and the main effect of population.

include the predicted probabilities: need to update gp_pred_popprxcnt to make it more modular

#+name: p_pred_dimred
#+begin_src R :exports :exports results :results output graphics file :file pred_dimred.pdf :width 6 :height 5.5
gp_
#+end_src
...
even if the "logic" (factor loadings) is right, the variable coverage of closed museums is just pretty bad -> PC1 scores for closed museums are lower than they should be
